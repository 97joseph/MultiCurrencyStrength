...
int PeriodIndexes[9] = {PERIOD_M1, PERIOD_M5, PERIOD_M15, PERIOD_M30, PERIOD_H1, PERIOD_H4, PERIOD_D1, PERIOD_W1, PERIOD_MN1};
string PeriodDesc[9] = {"M1", "M5", "M15", "M30", "H1", "H4", "D1", "W1", "MN1"};
bool PeriodEnabled[9] = {true, true, true, true, true, true, true, true, true};
...
{
    ChartSetSymbolPeriod(0, Pair, PeriodIndexes[Timeframe]);
    ChartNavigate(0, CHART_END, -(MAPeriod + 2));
}
...
double PopulateMatrixCell(int CurrencyIndex, string Currency, int PeriodIndex)
{
    ...
    for (int j = 0; j < ArraySize(AllPairs); j++)
    {
        ...
        {
            StartValue = iMA(AllPairs[j], PeriodIndexes[PeriodIndex], MAPeriod, 0, MAMethod, PRICE_CLOSE, BarsDifference);
            EndValue = iMA(AllPairs[j], PeriodIndexes[PeriodIndex], MAPeriod, 0, MAMethod, PRICE_CLOSE, 0);
            if (ShowAcceleration)
            {
                StartValuePrev = iMA(AllPairs[j], PeriodIndexes[PeriodIndex], MAPeriod, 0, MAMethod, PRICE_CLOSE, BarsDifference + 1);
...
void PopulatePairs()
{
    ...
    for (int i = 0; i < ArraySize(AllPairs); i++)
    {
        AllPairs[i] = StringConcatenate(_CurrPrefix, AllPairs[i], _CurrSuffix);
    }
}
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        for (int j = 0; j <= 10; j++)
        {
            ...
            else if ((j > 0) && (j < 10))
            {
                ...
                string Value = DoubleToStr(DoubleValue, 4);
...
void NotifyPossibleTrade()
{
    ...
    string EmailSubject = IndicatorName + " Notification ";
    string EmailBody = AccountCompany() + " - " + AccountName() + " - " + IntegerToString(AccountNumber()) + "\r\n" + IndicatorName + ": ";
    EmailBody += Setup;
    string AlertText = IndicatorName + " - ";
    AlertText += Setup;
